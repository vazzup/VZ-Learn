...
namespace vz_learn::data_manip
{
	...
	class data_t
	{
		 ...
		 * DESCRIPTION
		 * **********************
		 * Class which holds the data and remembers the data type
		 * **********************
		 * MEMBERS
		 * **********************
		...
		data_t(const long long int&, const std::string&,\
			const double&, const char&, const int& data_type);
		...
		private:
		friend std::ostream& operator<< (std::ostream &out, const vz_learn::data_manip::data_t &class_obj);
	};
}
